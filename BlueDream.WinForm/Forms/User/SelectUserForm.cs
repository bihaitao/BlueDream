...
namespace BlueDream.WinForm
{
    public partial class SelectUserForm : Form
    {
        ...
        public SelectUserForm(string p_ReturnKey)
        {
        ...
        private void LoadData(int p_PageSize, int p_PageIndex, string p_SearchKey, bool p_InitPage)
        {
            ...
            m_ApiUser.Parameters.Add("p_SearchKey", p_SearchKey);
            m_ApiUser.Parameters.Add("p_PageSize", p_PageSize);
            m_ApiUser.Parameters.Add("p_PageIndex", p_PageIndex);
            ApiPageResult<List<UserEntity>> m_ApiPageResult = m_ApiUser.GetListModelByPage();

            dgv_Main.DataSource = m_ApiPageResult.ResultObj;
            dgv_Main.AutoGenerateColumns = false;
            dgv_Main.Columns.Clear();
            InitDataGridViewColumn(dgv_Main, "UserID", "用户ID");
            dgv_Main.Columns["UserID"].Visible = false;
            InitDataGridViewColumn(dgv_Main, "UserName", "用户名");
            InitDataGridViewColumn(dgv_Main, "NickName", "显示名称");
        ...
        private void btn_Select_Click(object sender, EventArgs e)
        {
            ...
            List<UserEntity> m_UserList = (List<UserEntity>)dgv_Main.DataSource;

            UserEntity m_UserEntity = m_UserList[dgv_Main.SelectedRows[0].Index];

            CallBack_Event(m_ReturnKey, m_UserEntity);
